...
void PrintStackFrame(uint32_t eip,uint32_t ebp)
{
	while(ebp!=0)
	{
		int i=0;
		...
		while(i<nr_symtab_entry)
 		{
			if(((symtab[i].st_info&0xf)==STT_FUNC)&&(symtab[i].st_value<=eip)&&(symtab[i].st_value+symtab[i].st_size>eip)) break;
			i++;
 		}
//		printf("eip=%x  \n",eip);
		 printf("eip=%x  func:%s\n",eip,strtab+symtab[i].st_name);
		for(j=2;j<6;j++) printf("%x\t",swaddr_read(ebp+4*j,4));
		printf("\n");
...
uint32_t expr_value(char *args)
{
	...
	while(i<nr_symtab_entry)
 	{
        if(((symtab[i].st_info&0xf)==STT_OBJECT)&&(strcmp(strtab+symtab[i].st_name,args)==0)) break;	
		i++;
	} 
	if(i==nr_symtab_entry) return 0;
